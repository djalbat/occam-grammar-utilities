...
compoundExpression ::= expression_ (operator expression)+ ;
...
A bonus of this approach is that, if we are careful in removing or renaming the nodes in the parse tree corresponding to our reduced `expression_` rules, we get what is effectively the ideal parse tree to boot:
...
The `removeOrRenameReducedNodes(...)` function works on a node generated, say, as follows:
```
import { BasicLexer } from "occam-lexers";
import { BasicParser } from "occam-parsers";
import { removeOrRenameReducedNodes } from "occam-grammar-utilities";
